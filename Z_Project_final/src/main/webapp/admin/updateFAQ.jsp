...
<title>관리자 FAQ 등록</title>
	...
	<script>
		...
		$(function() {
			$("#inputCategory").val("${faqDetail.category}").prop("selected", true);
        });
	</script>
...
<body>
	...
	<br><br><br>
	<main>
		<form action="updateFAQ.do" method="post" name="faqInfo" onsubmit="return faqCheck()">
			<div id="main-css">
				<h3 class="center">자주하는 질문 수정</h3>
				<hr>
				...
				<div class="form-group">
					...
					<input type="text" class="form-control" name="question" placeholder="QUESTION" id="inputQuestion" value="${faqDetail.question }">
				</div>
				
				<%-- 답변 --%>
				<div class="form-group">
			        <label for="inputAnswer" class="form-label mt-4">답변</label>
			        <textarea class="form-control" name="answer" placeholder="ANSWER" id="inputAnswer" rows="3" style="margin-top: 0px; margin-bottom: 0px; height: 200px;">${faqDetail.answer }</textarea>
	            </div>
	            <br><br>
	            
	            <div class="center">
					<input type="submit" class="btn btn-primary" value="수정">
					<input type="hidden" name="no" value="${faqDetail.no }">
