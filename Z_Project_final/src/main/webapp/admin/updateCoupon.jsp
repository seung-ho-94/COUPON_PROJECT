...
<title>관리자 상품 수정</title>
	...
	<script>
		...
		<%-- 카테고리 값 셀렉 + 콤마 넣기 + submit할 때 상품 가격, 쿠폰 수량에 ,(콤마) 빼서 DB저장 --%>
		$(function() {
			$("#inputLocal").val("${couponDetail.local_category }").prop("selected", true);
			$("#inputCoupon").val("${couponDetail.coupon_category }").prop("selected", true);
				
			$("#inputPrice").val(comma($("#inputPrice").val()));
			$("#inputUploadCount").val(comma($("#inputUploadCount").val()));
...
<body>
	...
	<main>
	<br><br><br>
		<form action="updateCoupon.do" method="post" name="couponInsertInfo" enctype="multipart/form-data" onsubmit="return insertCheck()">
			<div id="main-css">
				<h3 class="center">상품 수정</h3>
				<hr>
				...
				<div class="form-group">
					...
					<div class="form-group">
						<div class="input-group mb-3">
							<input type="text" class="form-control" name="price" placeholder="PRICE" id="inputPrice"
								value="${couponDetail.price }" onkeyup="inputNumberFormat(this)">
				...
				<%-- 쿠폰 갯수 --%>
				<div class="form-group">
					<label class="col-form-label mt-4" for="inputUploadCount">쿠폰 갯수</label>
					<div class="form-group">
						<div class="input-group mb-3">
							<input type="text" class="form-control" name="upload_count" placeholder="UPLOAD COUNT" id="inputUploadCount"
								value="${couponDetail.upload_count }" onkeyup="inputNumberFormat(this)">
				...
				<div class="form-group">
					...
					<input type="date" class="form-control" name="first_date" data-placeholder="FIRST DATE" value="${couponDetail.first_date}">
					<input type="date" class="form-control" name="last_date" data-placeholder="LAST DATE" value="${couponDetail.last_date }">
				</div>
				
				<%-- 카테고리 --%>
				<div class="form-group">
					<label class="col-form-label mt-4">카테고리</label>
					<select class="form-control" name="local_category" id="inputLocal">
					...
					<select class="form-control" name="coupon_category" id="inputCoupon">
				...
				<div class="form-group">
					...
					<div class="grid">
						<label class="label-scope" for="file1">대표 이미지
							<c:choose>
								<c:when test="${empty couponDetail.attach_file1 }">
									<img class="img-content" src="/img/default-img.png" id="img1"/>
								</c:when>
								<c:otherwise>
									<img class="img-content" src="/img/${couponDetail.attach_file1 }" id="img1"/>
									<input type="hidden" name="attach_file1" value="${couponDetail.attach_file1 }">
								</c:otherwise>
							</c:choose>
						</label>
						...
						<label class="label-scope second" for="file2">서브 이미지
							<c:choose>
								<c:when test="${empty couponDetail.attach_file2 }">
									<img class="img-content" src="/img/default-img.png" id="img2"/>
								</c:when>
								<c:otherwise>
									<img class="img-content" src="/img/${couponDetail.attach_file2 }" id="img2"/>
									<input type="hidden" name="attach_file2" value="${couponDetail.attach_file2 }">
								</c:otherwise>
							</c:choose>
						</label>
						...
						<label class="label-scope" for="file3">서브 이미지
							<c:choose>
								<c:when test="${empty couponDetail.attach_file3 }">
									<img class="img-content" src="/img/default-img.png" id="img3"/>
								</c:when>
								<c:otherwise>
									<img class="img-content" src="/img/${couponDetail.attach_file3 }" id="img3"/>
									<input type="hidden" name="attach_file3" value="${couponDetail.attach_file3 }">
								</c:otherwise>
							</c:choose>
						</label>
				...
				<div class="center">
					<input type="submit" class="btn btn-primary" value="수정" id="btn-submit">
					<input type="hidden" name="coupon_key" value="${couponDetail.coupon_key }">
					<input type="button" class="btn btn-primary" value="취소" onclick="location.href='updateDeleteCoupon.do'">
				</div>
			</div>
		</form>
