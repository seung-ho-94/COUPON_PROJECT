			...
			<div class="container">
							...
							<c:if test="${not empty freeList }">
								<c:forEach var="vo" items="${freeList }">
									<tr>
										...
										<td><a href="selectOneFreeBoard.do?no=${vo.no }">${vo.title }</a></td>
										<td>${vo.id }</td>
										<td>${vo.regdate }</td>
										<td>${vo.hit }</td>
									</tr>	
				...
				<div >
					<button type="button" class="btn btn-sm btn-primary"
							onclick="location.href='insertFreeBoard.do'">글쓰기</button>
				</div>
				</c:if> 
				<!-- 페이징 처리 -->
				<div id="pagingDiv">
				...
				<form id="pagingFrm" name="pagingForm" action="selectFreeBoardList.do" method="get">
				...
				<div class="form-group row justify-content-center">
					<div id="search" class="w100" style="padding-right:10px">
						<form id="searchForm" action="selectFreeBoardList.do" method="get">
							<select name="type">
