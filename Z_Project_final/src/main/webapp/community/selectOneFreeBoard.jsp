		...
		<h2>자유 게시판 - 상세 보기</h2>
			<div class="container">
				<div class="my-3 p-3 bg-white rounded shadow-sm" style="padding-top: 10px">
					...
					<table class="table table-striped table-sm">
						...
						<tr>
							<th>등록일</th>
							<td>${freeDetail.regdate }</td>
						</tr>
						<tr>
							<th>조회수</th>
							<td>${freeDetail.hit }</td>
						</tr>
						<tr>
							<th>내용</th>
							<td>${freeDetail.content }</td>
						</tr>
						<c:if test="${freeDetail.file_name ne null}">
						<tr>
							<th>업로드 파일</th>
							<td><img src="../Resources/img/${freeDetail.file_name}" width="300"/></td>
						</tr>
						</c:if>
					</table>
					<c:if test="${sessionScope.loginId == freeDetail.id}">
					<div>		
						<button type="button" class="btn btn-sm btn-primary"
								onclick="location.href='updateFreeBoard.do?no=${freeDetail.no }'">수정</button>
						<button type="button" class="btn btn-sm btn-primary" 
								onclick="location.href='deleteFreeBoard.do?no=${freeDetail.no }'">삭제</button>
					</div>
					</c:if>
					</div>
				</div>
								...
								<td width="550">
									<div>${commList.content }
										<c:if test="${sessionScope.loginId == commList.id}">
											<form action="deleteFreeComment.do?no=${commList.no }" method="post">	
												...
												<input type="button" class="btn btn-sm btn-primary" onclick="updateFreeComment(${commList.no })" value="댓글 수정" style="float: right">		
											</form>
										</c:if>
									</div>
								</td>
			...
			<div class="my-3 p-3 bg-white rounded shadow-sm" style="padding-top: 10px">
				<form action="insertFreeComment.do" method="post">
					<input type="hidden" id="board_no" name="board_no" value="${freeDetail.no }" />
					<div class="row">
						<div class="col-sm-5">
			...
			<div style="text-align: center;">
				<button type="button" class="btn btn-sm btn-primary"
						onclick="location.href='selectFreeBoardList.do'">목록</button>
			</div>
			</div>
			</article>	
		</div>
...
<script>
	function updateFreeComment(no) {
		window.open("updateFreeComment.do?no="+no),
