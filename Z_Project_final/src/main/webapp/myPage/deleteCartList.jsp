          ...
          <div class="card">
            <div class="card-body">
              <nav class="nav flex-column nav-pills nav-gap-y-1">
                ...
                <a href="deleteCartList.do" class="nav-item nav-link has-icon nav-link-faded active">
					...
					<table class="table">
						...
						<tbody>
							<c:if test="${not empty cancelList }">
								...
								<tr>
									<td class="center">${cancelList.coupon_key }</td>
									<td><a href="couponDetail.do?coupon_key=${cancelList.coupon_key }">${cancelList.name }</a></td>
									<td>${cancelList.quantity }</td>
									<td>${cancelList.total_price }</td>
									<td>${cancelList.buy_date }
									&nbsp;&nbsp;&nbsp;

									<td>${cancelList.status }</td>
								</tr>	
								</c:forEach>
							</c:if>
							<c:if test="${empty cancelList }">
								<tr>
									<td colspan="8" class="center">취소 내역이 없습니다.</td>
								</tr>
							</c:if>
						</tbody>
					</table>
